 ...
 */
function grille_metaboxes_script( $hook ) {
	...
	wp_enqueue_style( 'grille-post-meta', get_parent_theme_file_uri( '/assets/css/metaboxes.css' ), false, '@@pkg.version', 'all' );
	...
	wp_enqueue_script( 'grille-post-meta', get_parent_theme_file_uri( '/assets/js/admin/post-meta.js' ), array( 'jquery' ), '@@pkg.version', true );
}
add_action( 'admin_enqueue_scripts', 'grille_metaboxes_script' );
	...
	foreach ( $meta_box['fields'] as $field ) {
		...
		switch ( $field['type'] ) {
				...
				jQuery(function($) {
					...
					$('#<?php echo esc_js( $field['id'] ); ?>_button').on('click', function(e) {
						...
						frame.toolbar.get('view').set({
							insert: {
								style: 'primary',
								text: '<?php esc_html_e( 'Insert', 'grille' ); ?>',
							...
							insert: {
								...
								click: function() {
									var models = frame.state().get('selection'),
										...
										this.el.innerHTML = '<?php esc_html_e( 'Saving...', 'grille' ); ?>';
			...
			?>
				...
				jQuery(function($) {
					...
					$('#bean_images_upload').on('click', function(e) {
						...
						//FIRST FRAME
						var options = {
							title: '<?php esc_html_e( 'Create Gallery', 'grille' ); ?>',
							state: 'gallery-edit',
							frame: 'post',
							selection: selection
						};
						//IF FRAME IS THERE
						if( frame || selection ) {
							options['title'] = '<?php esc_html_e( 'Edit Gallery', 'grille' ); ?>';
						}

						frame = wp.media(options).open();

						// CUSTOMIZE VIEWS
						frame.menu.get('view').get('cancel').el.innerHTML = '<?php esc_html_e( 'Cancel', 'grille' ); ?>';
						frame.menu.get('view').get('gallery-edit').el.innerHTML = '<?php esc_html_e( 'Edit Gallery', 'grille' ); ?>';
						...
						function overrideGalleryInsert() {
							frame.toolbar.get('view').set({
								insert: {
									style: 'primary',
									text: '<?php esc_html_e( 'Save Gallery', 'grille' ); ?>',
			...
			<?php
				...
				$thumbnail_output = '';
				$button_text      = ( $meta ) ? esc_html__( 'Edit Gallery', 'grille' ) : $field['std'];
			if ( $meta ) {
				$field['std']     = esc_html__( 'Edit Gallery', 'grille' );
				$thumbs           = explode( ',', $meta );
				$thumbnail_output = '';
			...
			case 'checkbox':
                ...
                <input type="checkbox" id="' . $field['id'] . '" name="bean_meta[' . $field['id'] . ']" value="on"' . $val . ' /><span>' . esc_html__( 'Yes, please do', 'grille' ) . '</span>';
				echo '</td>';
				break;
		}
