...
add_action( 'wp_ajax_nopriv_jm-post-like', 'grille_post_like' );
add_action( 'wp_ajax_jm-post-like', 'grille_post_like' );

function grille_post_like() {
	$nonce = $_POST['nonce'];
	if ( ! wp_verify_nonce( $nonce, 'ajax-nonce' ) ) {
		die( 'Nope!' );
	}

	if ( isset( $_POST['grille_post_like'] ) ) {

		$post_id         = $_POST['post_id']; // post id
		$post_like_count = get_post_meta( $post_id, '_likes', true ); // post like count
		...
		if ( is_user_logged_in() ) { // user is logged in
			...
			if ( ! grille_already_liked( $post_id ) ) { // like the post
				update_post_meta( $post_id, '_user_liked', $liked_USERS ); // Add user ID to post meta
				update_post_meta( $post_id, '_likes', ++$post_like_count ); // +1 count post meta
 ...
 */
function grille_already_liked( $post_id ) {
 ...
 * (4) Front end button
 */
function grille_get_post_like_link( $post_id ) {

	$like_count = get_post_meta( $post_id, '_likes', true );
	$count      = ( empty( $like_count ) || $like_count == '0' ) ? '0' : esc_attr( $like_count );

	if ( grille_already_liked( $post_id ) ) {
		$class = 'liked';
		$title = esc_attr( esc_html__( 'Unlike', 'grille' ) );
	} else {
		$class = '';
		$title = esc_attr( esc_html__( 'Like', 'grille' ) );
	}

	$output = '<div class="likes-wrapper ' . esc_attr( $class ) . '"><cite id="likes-count">' . esc_html( $count ) . '</cite> <a href="#" class="jm-post-like pulse-active" data-post_id="' . esc_attr( $post_id ) . '" title="' . esc_attr( $title ) . '">' . esc_html__( 'Likes', 'grille' ) . '</a></div>';
